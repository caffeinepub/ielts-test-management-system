...
import { ArrowLeft, FileDown, MessageSquarePlus, CheckCircle2, User } from 'lucide-react';
...
export default function ResponseDetailsPanel({ response, test, onClose }: ResponseDetailsPanelProps) {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      toast.error(message.includes('Unauthorized') ? 'Invalid credentials' : 'Failed to submit feedback');
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {!hasFeedback && !showFeedbackForm && (
              <Button variant="outline" size="sm" onClick={handleGiveFeedbackClick}>
          ...
          {hasFeedback ? (
            <div className="rounded-lg border bg-muted/40 p-4 space-y-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{response.feedback!.submittedBy}</span>
              </div>
              <p className="text-sm leading-relaxed">{response.feedback!.text}</p>
            </div>
          ) : showFeedbackForm ? (
            <div className="space-y-3">
              <Textarea
                placeholder="Write your feedback for this student's response..."
                value={feedbackText}
            ...
            <p className="text-sm text-muted-foreground">No feedback has been provided for this response yet.</p>
          )}
        </CardContent>
      </Card>
      ...
      <AuthModal
        ...
        onLogin={login}
        description="Please log in to submit feedback on this response"
      />
    </div>
  );
}
